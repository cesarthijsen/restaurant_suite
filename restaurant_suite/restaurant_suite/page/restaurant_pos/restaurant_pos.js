...
class RestaurantPOS {
  ...
  async load() {
    ...
    try {
      const response = await frappe.call({
        method:
          "restaurant_suite.restaurant_suite.page.restaurant_pos.restaurant_pos.get_pos_data",
      });
      ...
      frappe.msgprint(
        __(
          "Restaurant POS could not load. Check that Ice Cream POS is enabled."
        )
      );
    }
  }
  ...
  render() {
    this.$root = $("<div class='restaurant-pos'>").appendTo(
  ...
  render_step() {
    ...
    this.render_summary(
      $("<aside class='pos-panel pos-summary'>").appendTo($layout)
    );
  }
  ...
  render_progress($container) {
    ...
    this.flow.steps.forEach((step, index) => {
      const $button = $("<button type='button'>").text(
  ...
  render_options($container, step) {
    ...
    const instruction = step.required
      ? __("Required â€” choose {0}", [
          limit === 1
            ? __("one")
            : __("up to {0}", [limit || __("any number")]),
        ])
      : __("Optional");
    $container.append(
    ...
    step.options.forEach((option) => {
      ...
      $button.append(
        $("<small>").text(price ? format_currency(price) : __("Included"))
      );
    ...
    $(
      `<button class="btn btn-primary">${
        this.current_step === this.flow.steps.length - 1
          ? __("Review Order")
          : __("Next")
      }</button>`
    )
  ...
  render_summary($container) {
    $container.append(
      `<h4>${__(
        "Order Summary"
      )}</h4><div class="text-muted">${frappe.utils.escape_html(
    ...
    this.flow.steps.forEach((step) => {
      ...
      const labels = names.map(
        (name) =>
          step.options.find((option) => option.name === name)?.option_name ||
          name
      );
