...
class RestaurantPOS {
	...
	async load() {
		...
		try {
			...
			frappe.msgprint(__("Restaurant POS could not load. Check that Ice Cream POS is enabled."));
		}
	}

	add_styles() {
		if (document.getElementById("restaurant-pos-styles")) return;
		$("<style id='restaurant-pos-styles'>").text(`
	...
	render_step() {
		...
		this.$root.append(`
			<div class="pos-header">
				<div><h2>${frappe.utils.escape_html(this.flow.flow_name)}</h2><div class="text-muted">${frappe.utils.escape_html(this.flow.description || "")}</div></div>
	...
	render_progress($container) {
		...
		this.flow.steps.forEach((step, index) => {
			...
			if (index <= this.current_step) $button.on("click", () => { this.current_step = index; this.render_step(); });
	...
	render_options($container, step) {
		...
		const instruction = step.required
			? __("Required â€” choose {0}", [limit === 1 ? __("one") : __("up to {0}", [limit || __("any number")])])
			: __("Optional");
		$container.append(`<h3>${frappe.utils.escape_html(step.step_name)}</h3><div class="text-muted">${instruction}</div>`);
		...
		const $back = $(`<button class="btn btn-default">${__("Back")}</button>`).appendTo($actions);
		$back.prop("disabled", this.current_step === 0).on("click", () => { this.current_step -= 1; this.render_step(); });
		$(`<button class="btn btn-primary">${this.current_step === this.flow.steps.length - 1 ? __("Review Order") : __("Next")}</button>`)
			.appendTo($actions)
			.on("click", () => this.next());
	}
	...
	toggle_option(step, option_name) {
		...
		if (selected.includes(option_name)) selected = selected.filter((name) => name !== option_name);
		else if (step.selection_type === "Single") selected = [option_name];
		else {
			const limit = this.get_limit(step);
			if (limit && selected.length >= limit) {
				frappe.show_alert({ message: __("You can choose up to {0}.", [limit]), indicator: "orange" });
				return;
			}
	...
	get_limit(step) {
		...
		const source_step = this.flow.steps.find((row) => row.step_name === step.dynamic_limit_from_step);
	...
	next() {
		...
		if (count < minimum) {
			frappe.show_alert({ message: __("Please choose at least {0} option(s).", [minimum]), indicator: "red" });
			return;
		}
		this.current_step += 1;
		this.render_step();
	...
	render_summary($container) {
		$container.append(`<h4>${__("Order Summary")}</h4><div class="text-muted">${frappe.utils.escape_html(this.flow.menu_item_code)}</div>`);
		let has_selection = false;
		this.flow.steps.forEach((step) => {
			const names = this.selections[step.step_name] || [];
			if (!names.length) return;
			has_selection = true;
			const labels = names.map((name) => step.options.find((option) => option.name === name)?.option_name || name);
		...
		if (!has_selection) $container.append(`<p class="pos-empty">${__("Your selections will appear here.")}</p>`);
	}
